...
- (void)seekTo:(float)value {
    ...
    // FIXME: 修复播放器无法播放小数位的value，比如total是23，time为22.5。所以time的最大值为 totalTime - 1
    time = MIN(time, self.totalTime - 1);
    self.isSeeking = YES;
    if (self.seekToTimeBlock) {
        __weak typeof(self) weakSelf = self;
        self.seekToTimeBlock(time, ^(BOOL finished) {
...
- (void)showSmallSlider {
    // FIXME: 播放状态下不显示进度条,只有在暂停或者拖拽才显示
